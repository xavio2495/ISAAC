...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${BASE_URL}/${chainId}/approve/allowance?tokenAddress=${tokenAddress}&walletAddress=${walletAddress}`;
    ...
    if (!response.ok) {
      ...
      console.error('1inch allowance API error:', errorText);
      return NextResponse.json(
    ...
    console.error('Allowance proxy error:', error);
    return NextResponse.json(
